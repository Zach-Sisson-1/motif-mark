...
def get_args():
	parser = argparse.ArgumentParser(description="Script that inputs a fasta file and motif file and outputs an image with motifs labeled on each gene")
...
class Uniq_Motif:
	"""
	Motif class that takes in a motif and can search for that motif in a supplied gene, outputing a dictionary containing motif positions on the gene
	"""
	def __init__(self, motif):
		...
		for nucleotide in self.motif_lower:
			if nucleotide == "r":
				...
				regex += nucleotide
		regex += '))'					
		self.regex = regex	#	Assign unique regex expression for motif
	
	def gene_search(self,Gene):
		"""
		Takes in a gene object and searches for the motif in the gene, returning a dictionary  
		 containing the unique motifs as keys and tuple coordinates as values representing the position where the motif was found.
		""" 
		#	initialize dict which will house motif as keys and location hits as values 
		match_dict = {}

		#	Create iterobjects for the motif and locate all the motif hits within the given sequence using regex				
		match_dict[self.motif] = re.finditer(self.regex,Gene.gene_seq_lower)

		#	Use only the the index locations of the matches (i.e span)
		index_dict = {}	
		for motif,iterobj in match_dict.items():
			lst = [obj.span() for obj in list(iterobj)]
			index_dict[motif] = lst
		#	update the span to reflect motif length (i.e replace second value in the tuple with the appended length).
		index_dict2 =	{}
...
class Gene:
	...
	def __init__(self,gene_seq, header):
		self.gene_seq = gene_seq	# gene sequence
		self.gene_seq_lower = gene_seq.lower()   #	hold lowercase gene seq string for motif searching
		self.header = header					#	header for FASTA seq 
		self.length = len(self.gene_seq_lower)  # gene seq length 
...
with open(args.fasta_file, "r") as fasta_file:
	gene_dict = {} 				#	empty dict that will contain fasta headers as keys and corresponding genes as values
...
class Drawing:
	...
	def __init__(self,master_dict) -> None:
		...
		self.X = int(counter+(0.2*counter))								# longest gene + 20%
	...
	def draw(self):	
		...
		for gene,lst in master_dict.items():
			...
			ctx.move_to(self.gapx,yslider)
			ctx.line_to(lst[0].pre_exon_length+self.gapx,yslider) 
			ctx.stroke()
			ctx.fill()
			...
			x0 = lst[0].pre_exon_length+self.gapx
			y0 = yslider-(hgt/2)
			#	draw exon rectangle
			...
			#	draw post-exon						
			ctx.move_to((lst[0].pre_exon_length+lst[0].exon_length+self.gapx),yslider)
			ctx.line_to((lst[0].pre_exon_length+lst[0].exon_length+lst[0].post_exon_length+self.gapx),yslider) 
			ctx.stroke()
			ctx.fill()
	...
	### At end of loop Draw Legend box
...
#call the objects method to create drawing
surface = drawing.draw()

#Specify output file name based in argsparse
